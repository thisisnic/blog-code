...
Smart pointers are a modern version of pointers which is a class template and allows memory management to be done via C++ scoping rather than needing to be done manually.

They can be declared using `std::uniqueptr`, `std::sharedptr`, and `std::weakptr`.

A `sharedptr` is a smart pointer which can be managed by multiple sources.  You should use `make_shared` to create it.
